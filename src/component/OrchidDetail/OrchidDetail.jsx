...
import { FaStar, FaHeart, FaChevronDown, FaChevronUp, FaLeaf } from "react-icons/fa";
...
const OrchidDetail = () => {
        ...
        return (
            <div className="flex items-center">
                {[...Array(5)].map((_, index) => (
                    <FaStar
                        key={index}
                        className={`${index < Math.floor(rating) ? "text-yellow-400" : "text-gray-300"} w-5 h-5`}
                    />
                ))}
    ...
    return (
        <div className="container mx-auto p-4 mt-4 mb-12 max-w-7xl">
            <div className="mb-4 w-36 p-2 bg-rose-400 hover:bg-rose-700 dark:bg-blue-400 dark:hover:bg-blue-700 text-white rounded-lg flex items-center gap-2">
                <FaArrowLeft className="inline-block" />
                <Link to={'/'}>Back to home</Link>
            </div>
            <div className="flex flex-col lg:flex-row gap-8 bg-white overflow-hidden">
                {/* Image Only*/}
                ...
                <div className="lg:w-1/2 p-6 flex flex-col gap-6">
                    {/* 1st Section*/}
                    <div>
                        <h1 className="text-3xl font-bold text-gray-800 mb-4">{orchidData.name}</h1>
                    ...
                    <div>
                        ...
                        <div className={`relative ${!isExpanded ? "max-h-12" : "max-h-full"} overflow-hidden transition-all duration-300`}>
                            <p className="text-gray-700">{orchidData.description}</p>
                        </div>
                        <button
                            onClick={() => setIsExpanded(!isExpanded)}
                            className="mt-2 flex items-center gap-2 text-rose-600 hover:text-rose-700 transition-colors"
                        >
                            {isExpanded ? (
                                <>
                    ...
                    <div className="flex items-center justify-between mt-4">
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-rose-200 hover:bg-rose-600 transition-colors"
                        >
